...
{ 
      ...
      {  
             pair<ll, ll> next = p;
            if(s[i] == 'S'){--next.second;}
            else if(s[i] == 'N'){++next.second;}
            else if(s[i] == 'W'){--next.first;}
            else if(s[i] == 'E'){++next.first;}

            ll val = 5- 4* st.count(make_pair(p,next));
            ans=ans+val;
            st.insert(make_pair(p,next));
            st.insert(make_pair(next,p));
