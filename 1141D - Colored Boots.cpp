...
void solve()
{
    ...
    fr(i,0,26)
    {
        ...
        vi tc=ta[i];
        int k;
        for( k=0;k<sc.size() && k<tc.size();k++)
        ans.pb(mpa(sc[k],tc[k]));
        ll j=k;
        while(j<sc.size())
        {sl.pb(sc[j]);j++;}
        j=k;
    ...
    ll sqi=0,tqi=0;
    for(int i=0;i<sl.size() && tqi<tq.size();i++)
    {
        ans.pb(mpa(sl[i],tq[tqi]));
        tqi++;
    }
    for(int i=0;i<tl.size() && sqi<sq.size();i++)
    {
        ans.pb(mpa(sq[sqi],tl[i]));
        sqi++;
    }

    while(tqi<tq.size() && sqi<sq.size())
    {
        ans.pb(mpa(sq[sqi],tq[tqi]));
        sqi++;tqi++;
    }
