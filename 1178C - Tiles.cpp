...
{   
    // for (0,0) 4 choices
    //for h-1 rows 2 choices for each(0,0)choices =2^(h-1)
    //similarly for column= 2^(w-1)
    //and rest all be fixed by the above cell and left cell
    //therfore total = (2)^(w+h-2)*4= 2^(w+h-2+2)
